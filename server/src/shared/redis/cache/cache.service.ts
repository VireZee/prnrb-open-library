...
export class CacheService extends RedisService {
    ...
    async createCollection(keyName: string, user: { id: string }): Promise<Collection[]> {
        ...
        const cache = await this.client().json.GET(key)
        ...
        await this.client().json.SET(key, '$', books)
    ...
    async updateCollection(keyName: string, user: { id: string }): Promise<void> {
        ...
        await this.client().json.SET(key, '$', this.formatterService.formatBooksMap(updatedBooks))
        await this.client().EXPIRE(key, 86400, 'NX')
